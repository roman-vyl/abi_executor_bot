...
- [x] 1.1 Add a strict ambiguous-CREATE predicate covering only
  ...
  close identity.
- [x] 1.2 Add strict Bybit server-time decoding and a post-observation freshness check
  requiring non-negative binding age strictly below the documented seven-day window.
- [x] 1.3 Reuse the existing paginated exact-own execution resolver so found execution
  blocks absence and ambiguous/incomplete execution evidence fails closed.

## 2. Full-budget recovery outcome

- [x] 2.1 Accumulate `entry_order_not_found` eligibility across all three existing
  recovery attempts instead of returning after the first order `not_found`.
- [x] 2.2 Require every attempt to have clean exact-order absence, complete clean
...
- [x] 2.3 Preserve positive-state priority so a later live, terminal, or fill finding
  supersedes any earlier clean absence.
- [x] 2.4 Add the fifth typed/HTTP result with all conditional fields null and keep the GET
  free of exchange writes and absence persistence.

## 3. Corrective CANCEL safety

- [x] 3.1 For `desired_entry:null` against the ambiguous-CREATE shape, require the same
...
- [x] 3.2 Preserve existing positive behavior: cancel an exact live order; honor terminal
  or fill evidence; return safe error without absence persistence for ambiguous or
  aged-out evidence.
- [x] 3.3 Prove other record shapes and ordinary cancellation/terminal confirmation retain
  existing behavior.

## 4. Focused verification

- [x] 4.1 Test all structural exclusions: applied, pending-cancel, create-failed, legacy
  actions, null identity, invalid/future binding time, durable observation/fill/close.
- [x] 4.2 Test first/second attempt absence followed by later live/fill, and an early
  failed/malformed/mismatched attempt followed by empty reads.
- [x] 4.3 Test execution found, execution ambiguity, incomplete pagination, and complete
  no-execution evidence.
- [x] 4.4 Test ages just below, exactly at, and beyond seven days using validated Bybit
  server time for both GET and corrective CANCEL.
- [x] 4.5 Test that an observation obtained before expiry cannot produce
  `EntryPackageAbsent` when the corrective CANCEL completes at/after expiry.
- [x] 4.6 Test that a same-side sibling position does not suppress either the fifth state
  or corrective absence, while opposite-side and failed/malformed aggregate evidence do.
- [x] 4.7 Run `npm test`, `npm run typecheck`, and strict OpenSpec validation.

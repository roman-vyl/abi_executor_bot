...
`entry_order_not_found` SHALL mean only that the bounded exact-own-order realtime and
history reads both completed cleanly and did not contain the expected identity. It SHALL
NOT be documented or encoded as a terminal state or as proof that the original CREATE
never existed, never reached the exchange, or never filled.
...
- **WHEN** ABI cleanly observes the exact own order identity absent from bounded realtime
  and history reads
...
A clean exact-own-order absence is no longer part of this error class: when both bounded
order queries complete successfully and report the expected identity absent, ABI SHALL
return `entry_order_not_found`. An empty aggregate position query is neither required nor
sufficient for that outcome.
...
#### Scenario: Clean exact-own-order absence returns a distinct success
- **WHEN** the bounded realtime and history reads both complete cleanly and do not contain
  the exact own order identity
- **THEN** ABI returns HTTP `200` with `recovery_state:"entry_order_not_found"`

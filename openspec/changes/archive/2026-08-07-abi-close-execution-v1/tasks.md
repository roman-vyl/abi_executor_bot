...
- [x] 1.1 Add `"terminal_closed"` to `EntryPackageExecutionStatus` and its `STATUSES` set in
...
- [x] 1.2 Confirm `EntryPackageCorrelationRepository`'s replay-time conflict check
...
- [x] 2.1 Add a close-specific neutralization classification, built from `packageConfirmation.ts`'s
...
- [x] 2.2 Add a small neutralization helper (e.g. alongside `packageConfirmation.ts`) that, given a
...
- [x] 3.1 In the close pipeline, call `bybit.queryPositionForInstrument` directly for the pair's
...
- [x] 3.2 Wire the already-defined `executeMarketCloseOrder` (`execution.ts`) and
...
- [x] 4.1 Add `src/services/close/closeApplicationService.ts` implementing three paths from pair
...
- [x] 4.2 Reuse the existing `KeyedMutex` instance and `correlationRecordKey`, mirroring
...
- [x] 4.3 On the terminal write (either path), set `status: "terminal_closed"` and clear
...
- [x] 5.1 In `EntryPackageApplicationService.handleNonNullDesiredEntry`, add the same fail-closed
      branch already used for `terminal_unfilled` when `record.status === "terminal_closed"`.
- [x] 5.2 In `EntryPackageApplicationService.handleNullDesiredEntry`, return the existing absent
...
- [x] 6.1 Add a `CloseApplicationServicePort` to `positionManagementRoutes.ts`, mirroring
      `ProtectionApplicationServicePort`, and thread it through `handlePositionManagementRoutes` and
      `handleClose`.
- [x] 6.2 Construct `CloseApplicationService` in `app/server.ts` and pass it into
      `handlePositionManagementRoutes`, alongside the existing `protectionApplicationService`.

## 7. Tests

- [x] 7.1 `CloseApplicationService` unit tests covering: unknown pair; already `terminal_closed` pair
...
- [x] 7.2 `position-scope-exclusivity`-facing tests: a new entry request for a former `terminal_closed`
...
- [x] 7.3 `entry-package-execution`-facing tests: a non-null `desired_entry` request against a
...
- [x] 7.4 `open-position-resolution`-facing test: `GET .../open-position` for a `terminal_closed` pair
      returns `position_open: false` without querying the exchange.
- [x] 7.5 Idempotency tests: a repeated `DELETE` after `terminal_closed` — reached via the full
...
- [x] 7.6 Concurrency tests: a close command and a concurrent entry-package or protection command for
...
- [x] 7.7 Update `positionManagementRoutes.test.ts`'s close stub-behavior test to exercise the real
      (fake-backed) service path.

## 8. Verification

- [x] 8.1 Run `npm test`, `npm run typecheck`, `npm run build`.
- [x] 8.2 Run the repo's existing OpenAPI validation for `docs/openapi/abi-position-management-api-v1.json`
      and confirm it is unchanged (no public contract change in this OpenSpec).
- [x] 8.3 Run `git diff --check`.
- [x] 8.4 Review the diff: no new public DTO, route, or error code; no account-wide/symbol-wide cancel

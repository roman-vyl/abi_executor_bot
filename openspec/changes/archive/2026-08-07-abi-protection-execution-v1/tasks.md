...
- [x] 1.1 In `validateProtectionCommand` (`positionManagementApi.ts`), replace `isExactDecimalText`
...
- [x] 2.1 Add `stopLoss`/`takeProfit` to `ValidatedOpenPositionRow` as raw, optional exact-decimal
      strings (Decision 6). No change to `evaluatePositionQueryResponse`'s existing outcomes.
- [x] 2.2 Replace `SetTradingStopInput`/`BybitAdapter.setTradingStop` with a correctly-shaped
...
- [x] 2.3 Add `executeProtectionUpdate` to `src/execution/execution.ts`, mirroring
      `executeEntryOrder`'s live-guard gating.

## 3. Reuse the live-position gate

- [x] 3.1 Split `OpenPositionResolutionService.resolve()` into its HTTP-shaping wrapper plus an
...
- [x] 4.1 Add `src/services/protection/protectionApplicationService.ts`: durable-absence shortcut →
      ownership re-check → live-position gate (3.1) → write (2.2/2.3) → bounded read-back (Decisions
      2, 3, 6). No correlation-store write.
- [x] 4.2 Reuse the existing `KeyedMutex` instance and `correlationRecordKey`; do not use
      `scopeMutex` (Decision 7).
- [x] 4.3 Gate on the existing entry-package readiness signal.

## 5. Wiring

- [x] 5.1 Construct `ProtectionApplicationService` in `app/server.ts` and pass it into
      `handlePositionManagementRoutes`.
- [x] 5.2 Update `handleProtection` to call the service instead of unconditionally returning
      `internal_error`.

## 6. Tests

- [x] 6.1 `ProtectionApplicationService` unit tests: unknown pair; durably-absent pair (no exchange
...
- [x] 6.2 Update `positionManagementRoutes.test.ts`'s stub-behavior test to exercise the real
      (fake-backed) service path.
- [x] 6.3 Concurrency test: protection vs. concurrent entry-package replace for the same pair never
      interleave; different pairs proceed independently.
- [x] 6.4 Confirm `openPositionResolutionService.test.ts` and `positionManagementApi.test.ts` pass
      unchanged (other than the 1.1 rejection additions).

## 7. Verification

- [x] 7.1 Run `npm test`, `npm run typecheck`, `npm run build`.
- [x] 7.2 Review the diff: no new public DTO, route, or error code beyond the tightened price

...
- [x] 1.1 Add open-position response types to a new domain module (e.g.
...
- [x] 1.2 Add `OpenPositionErrorCode = "validation_failed" | "unknown_trade_cycle_binding"
...
- [x] 1.3 Add path-decoding types/helpers for the two opaque path parameters, reusing the
...
- [x] 1.4 Add an internal `OpenPositionResolutionResult` type (or reuse an
...
- [x] 2.1 Add an open-position resolution service (e.g.
...
- [x] 2.2 Implement the missing-record branch: return `unknown_trade_cycle_binding`, never
      `position_open: false` (design.md Decision 1).
- [x] 2.3 Implement the three-bucket status classification exactly as design.md Decision 2
...
- [x] 2.7 Implement final response assembly: no live row (or all rows `size == 0`) →
...
- [x] 2.8 Map every adapter-layer failure from section 3 (transport, malformed envelope,
      ...
      `position_open: false`.
- [x] 2.9 Add a short module-level comment on the resolution service documenting the V1
...
- [x] 3.1 Add `category?: string` to `GetOpenPositionsInput`
      ...
      4, closes gap G2).
- [x] 3.2 Add `avgPrice?: string` and `openTime?: number` to `BybitPosition`
...
- [x] 3.3 Add a new adapter method (e.g. `queryPositionForInstrument({ category, symbol })`
...
- [x] 3.4 Add a typed error/result type for 3.3's failure branches so the application
      service (2.8) can map each to `internal_error` without inspecting adapter
      internals.

## 4. HTTP route and path decoding

- [x] 4.1 Add a route matcher (e.g. `matchOpenPositionRoute` in a new
...
- [x] 4.2 Add `handleOpenPositionRoutes(...)` following the same
...
- [x] 4.3 Wire the matched, decoded path values into the section-2 resolution service and
      write its result using the same `writeJson`/result-writing helper the entry-package
      route uses.

## 5. Composition/readiness wiring

- [x] 5.1 In `src/app/server.ts`, construct the section-2 resolution service using the
      ...
      Decision 8).
- [x] 5.2 Add `handleOpenPositionRoutes` to the route chain in `server.ts`, gated on the
...
- [x] 6.1 Add a new OpenAPI 3.1 operation for `GET
...
- [x] 7.1 Unit test the three-bucket status classification (section 2.3) for all eight
      `EntryPackageExecutionStatus` values individually.
- [x] 7.2 Unit test the missing-record branch (2.2), the category gate (2.4), and the
      side-match branch (2.6), including both the pass and fail cases for each.
- [x] 7.3 Unit test response assembly (2.7): open with partial fill, open with full fill,
      closed via no live row, closed via all-`size==0` rows.
- [x] 7.4 Unit test the typed Bybit query (3.3) directly against constructed raw
...
- [x] 7.5 Unit test the route matcher (4.1) for valid paths, missing/empty segments, and
      malformed percent-encoding, mirroring the existing `matchEntryPackageRoute` test
      coverage style.

## 8. Route/contract tests

- [x] 8.1 Contract-test the success DTO shape and cross-field invariant (closed object,
      no extra fields, both-null vs both-non-null) against
      `abi-open-position-lookup-api`'s spec scenarios.
- [x] 8.2 Contract-test every documented error response (`validation_failed`,
...
- [x] 8.3 Contract-test that no response ever contains `404`, raw Bybit response bodies,
      internal exception details, or a stack trace.
- [x] 8.4 Route-test readiness gating: request before `EntryPackageReadiness` succeeds
      returns `internal_error` without attempting correlation lookup or an exchange call.

## 9. Fake-Bybit integration tests

- [x] 9.1 Integration test the full happy path against a fake Bybit backend: correlation
...
- [x] 9.2 Integration test partial fill (`size` less than the record's intended order
      quantity) still reports `position_open: true`.
- [x] 9.3 Integration test closed-position paths: `absent`/`terminal_unfilled` records
...
- [x] 9.4 Integration test unresolved-status records (`pending_create`, `create_failed`,
      `unknown`) return `internal_error` without invoking the fake Bybit backend.
- [x] 9.5 Integration test unsupported category (non-`linear` `exchange_category` on the
      record) returns `unsupported_exchange_scope` without invoking the fake Bybit
      backend.
- [x] 9.6 Integration test exchange-derived failures against the fake backend: simulated
...
- [x] 9.7 Integration test the missing-record case end-to-end: a pair with no correlation
...
- [x] 10.1 Run `npm test`, `npm run typecheck`, and `npm run build`; fix any failures.
- [x] 10.2 Run `openspec validate abi-open-position-lookup-v1 --strict` and `openspec
      validate --all --strict`; fix any errors.
- [x] 10.3 Confirm no diff to `EntryPackageCorrelationRepository`'s schema/index, the

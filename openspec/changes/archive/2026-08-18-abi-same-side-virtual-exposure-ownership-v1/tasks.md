...
- [x] 1.1 In `EntryPackageApplicationService.createOrder()`, inside the existing
...
- [x] 1.2 If any remaining (other-pair) active record has `desired_entry === null`, classify as
      `"corrupt"` — design.md Decision 1.
- [x] 1.3 Otherwise classify as `"empty"` (no other active records), `"same_side"`, or `"opposite_side"`
...
- [x] 1.4 Add the temporary production guard, clearly commented and naming this change and Change 8 by
...
- [x] 2.1 No code change. Add a doc comment at `findOwnerByScope()`'s definition noting it is no longer a
...
- [x] 3.1 In `rebuildScopeIndexFromReplay()`, add a local (function-scoped, not persisted)
...
- [x] 3.2 An active record whose `desired_entry?.side` is `undefined` → readiness failure. An active
...
- [x] 4.1 In `ProtectionApplicationService.process()`, replace the `findOwnerByScope()`-based ownership
...
- [x] 4.2 If the requested pair is not among the returned active records, fail closed with the existing
      `internalErrorResult()` — design.md Decision 4.
- [x] 4.3 If more than one active record is returned, fail closed with the new
...
- [x] 4.4 Exactly one active record, matching the requested pair: fall through to the existing,
      byte-for-byte-unchanged `determine()` → already-satisfied short-circuit → write → read-back flow.

## 5. New error code

- [x] 5.1 Add `"shared_scope_protection_unsupported"` to `PositionManagementErrorCode`
...
- [x] 6.1 `position-scope-exclusivity/spec.md`: document the internal mechanism swap (single-pointer →
...
- [x] 6.2 `protection-execution/spec.md`: MODIFIED "The pair is classified before any exchange call"
...
- [x] 6.3 `abi-position-management-api/spec.md`: MODIFIED closed error-code table, one new row, noted as
      currently unreachable in production for the same reason.

## 7. Tests (design.md "Required tests", full list)

- [x] 7.1 Admission classification correctness (synthetic fixtures): empty / same-side / opposite-side /
      corrupt, all four outcomes distinguished.
- [x] 7.2 Admission self-exclusion (synthetic): a requesting pair's own record is excluded from the
      "other" set even when a genuinely different same-side sibling is also active.
- [x] 7.3 Admission temporary guard (end-to-end): two different pairs, same side, racing one scope —
...
- [x] 7.4 Self-repeat regression (end-to-end, existing coverage) continues to pass unmodified.
- [x] 7.5 Replay: two same-side active records for one scope (synthetic) → success, both active.
- [x] 7.6 Replay: mixed-side active records for one scope → readiness failure.
- [x] 7.7 Replay: an active record with no usable `desired_entry.side` → readiness failure.
- [x] 7.8 Durable close of a synthetically-seeded owner does not affect a synthetically-seeded sibling's
      continued active ownership.
- [x] 7.9 Protection with two synthetically-seeded active owners → `shared_scope_protection_unsupported`,
      zero exchange writes.
- [x] 7.10 Protection with exactly one active owner (matching pair) → full regression, unchanged.
- [x] 7.11 Protection where the requested pair is not among active records → existing `internal_error`.
- [x] 7.12 Full regression of `protectionApplicationService.test.ts`'s existing single-owner scenarios.
- [x] 7.13 Full regression of `entryPackageApplicationService.test.ts` in its entirety — explicitly
...
- [x] 8.3 Build clean.
- [x] 8.4 Diff review confirms no file outside this change's stated Impact was touched, and specifically

...
- **BREAKING**: close uses one cycle-owned, pair-scoped algorithm for one or many same-side owners. Close quantity comes from that cycle's attributable filled exposure, close-order recovery uses its stable own identity, and success requires both own exposure closure and own attached Partial children becoming inactive/terminal (or safely absent where the existing attribution contract permits).
...
- `close-execution`: make pair-scoped, own-fill-derived close and own-child cleanup the sole close algorithm regardless of owner count.

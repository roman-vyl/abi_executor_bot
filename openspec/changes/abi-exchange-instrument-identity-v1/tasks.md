...
- [ ] 1.2 Unit test the two accepted grammars: `BTCUSDT.P` → `{symbol: BTCUSDT, category:
      ...
      spot}`.
- [ ] 1.3 Unit test rejection of every malformed shape per design.md Decision 1 —
...
- [ ] 3.2 Add `exchange_category: string` to `EntryPackageExecutionRecord` and
...
- [ ] 3.3 Update every existing-binding payload built inline in
...
- [ ] 3.4 Change `InstrumentTradingRulesProvider.getRules` to `getRules(symbol, category)`
...
- [ ] 3.5 Change `BybitAdapter.getInstrumentInfo` (interface, `RestBybitAdapter`, and
...
- [ ] 3.6 Update any test fixtures/doubles that construct `EntryPackageOrderInput`, call
...
- [ ] 4.1 Test that amend/cancel/realtime-query/history-query payloads for an existing
...
- [ ] 4.2 Run `npm test`, `npm run typecheck`, `npm run build`.
- [ ] 4.3 Run `openspec validate --all --strict`.
- [ ] 4.4 Confirm no diff to `src/domain/entryPackageApi.ts`, `src/domain/signals.ts`,

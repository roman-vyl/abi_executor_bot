...
`operation_completed` SHALL be used whenever the application operation runs to a normal, typed result — this includes handled business-negative outcomes (e.g. `position_not_open`, `unknown_trade_cycle_binding`, `unsupported_exchange_scope`) and is independent of the HTTP status code the caller ultimately receives. `operation_failed` SHALL be reserved for `internal_error` outcomes: an unexpected thrown exception or rejected promise reaching the instrumentation boundary, not an anticipated business result.
...
#### Scenario: Internal error becomes a failed, error-level event
- **WHEN** any of the four execution operations throws or rejects unexpectedly at the instrumentation boundary

...
correlation state that lets ABI tell "no position was ever established" apart from "a position
existed and was deliberately, provably closed" — the second of which must never again accept a new
entry for the same trade cycle.
...
- Wire `DELETE .../open-position` to real execution: neutralize the trade cycle's current entry order
  ...
  never before.
- Introduce a new durable trade-cycle outcome, distinct from the existing "never entered" outcome,
  for a trade cycle that held (or could have held) real exposure and has now been provably and
  completely closed. That distinction is what stops a closed trade cycle from being revived by a
  later entry-package request for the same pair.
...
- Trading safety: a trade cycle is only ever reported closed after its current entry order is proven
  unable to add exposure and its live position is proven zero, both confirmed immediately before the
  terminal record is written; the physical scope is never freed ahead of that durable write, so a
  different pair can never be let onto the same scope while this one might still hold exposure.

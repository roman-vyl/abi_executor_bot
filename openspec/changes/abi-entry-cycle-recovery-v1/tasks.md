...
- [ ] 1.1 Remove `replaceAmend` from `entryPackageApplicationService.ts`; repoint the
...
- [ ] 1.2 Remove `replaceCancelAndCreate` and the `pending_action: "cancel_and_create"`
      dispatch branch in `resendPendingAction`.
- [ ] 1.3 Remove `amendEntryOrder` from `execution.ts` and `amendOrder` from
      `bybitAdapter.ts` (both become unused).
- [ ] 1.4 Remove or update any test fixtures/mocks that stub the removed amend/BybitAdapter
      surface.

## 2. Fix `cancelLiveOrder` ambiguity handling

- [ ] 2.1 In `cancelLiveOrder`'s transport-exception catch block, durably record
      `status: "unknown"` (preserving `pending_action: "cancel"`) before returning a safe
      error, matching `createOrder`'s catch block.
- [ ] 2.2 Add preflight revalidation to `handleNullDesiredEntry`: before dispatching into
...
- [ ] 3.1 Implement the four-state classification (`entry_order_live`, `position_open`,
...
- [ ] 3.2 `terminal_without_fill`/`terminal_after_fill` require positive evidence
...
- [ ] 3.3 Implement dual-positive-confirmation resolution: `position_open`,
...
- [ ] 3.4 Use the same bounded realtime-plus-history query pattern
      `confirmEntryPackage` already uses; no special `startTime`/`endTime` windowing is
      introduced by this change.
- [ ] 3.5 Include the correlation record's `desired_entry` + `calculated_quantity` as
      `AppliedEntryPackage` in the response for `entry_order_live`/`position_open`; omit
      it for the other two states.
- [ ] 3.6 Missing correlation record for the requested pair returns `422
...
- [ ] 4.1 Add `GET /v1/strategy-instances/{strategy_instance_id}/trade-cycles/{trade_cycle_id}/recovery-state`,
      routed alongside `openPositionRoutes.ts`, delegating to the new resolution service.
- [ ] 4.2 Apply the same opaque-path-parameter handling (percent-decoding, non-empty
      validation, `422 validation_failed`) already used by the open-position lookup
      route.
- [ ] 4.3 Add the OpenAPI document for the new endpoint under `docs/openapi/`.

## 5. Regression coverage

- [ ] 5.1 Cancel-only replace: a changed non-null `desired_entry` against an existing
      binding cancels and returns `entry_package_absent`, and never creates a second
      order.
- [ ] 5.2 `cancelLiveOrder` transport failure records `status: "unknown"`.
- [ ] 5.3 Repeat cancel-intent PUT against a still-live order resends cancel only after
...
- [ ] 5.4 Recovery-state resolution: each of the four states from agreeing dual evidence,
      including a `PartiallyFilled` order confirmed by an open position resolving
      `position_open`.
- [ ] 5.5 Recovery-state resolution: a clean-but-empty result everywhere (no live order,
...
- [ ] 5.6 Recovery-state resolution: contradictory or incomplete evidence fails safe —
...
- [ ] 5.7 Missing correlation record returns `422 unknown_trade_cycle_binding`, and this
...
- [ ] 5.8 `AppliedEntryPackage` is present only for `entry_order_live`/`position_open`.

## 6. Verification

- [ ] 6.1 Run `npm test`, `npm run typecheck`, and the OpenAPI verification command.
- [ ] 6.2 Run strict OpenSpec validation (`npm exec -- openspec validate --all --strict`).

...
export type DesiredProtectionStateResolutionFailure = "no_authoritative_qty" | "surrogate_unrepresentable";
...
// non-null, else a computed, tick-valid, instrument-bounds-clamped
// surrogate anchored to this cycle's own immutable
// desired_entry.planned_entry_price. Both legs always carry the same qty.
export async function resolveDesiredProtectionState(input: {
  ...
  if (command.takePrice !== null) {
    ...
    const surrogate = computeSurrogateTakePrice({
    ...
    takeTriggerPrice = surrogate.price;
  }

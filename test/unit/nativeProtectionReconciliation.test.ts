...
// ---- computeSurrogateTakePrice / clampToInstrumentBounds (task 4.3) ----

test("computeSurrogateTakePrice is deterministic and idempotent across repeated calls with unchanged inputs", () => {
  const input = { plannedEntryPrice: "100000", side: "long" as const, tickSize: "0.5", minPrice: "0.5", maxPrice: "1999999.98" };
...
test("long produces a price strictly above plannedEntryPrice, short strictly below, when unclamped", () => {
  const longResult = computeSurrogateTakePrice({
  ...
  assert.ok(longResult.ok);
  if (longResult.ok) {
    assert.equal(longResult.price, "150000");
  }

  const shortResult = computeSurrogateTakePrice({
    plannedEntryPrice: "100000",
    side: "short",
    tickSize: "0.5",
    minPrice: "0.5",
  ...
  assert.ok(shortResult.ok);
  if (shortResult.ok) {
    assert.equal(shortResult.price, "50000");
  }
});

test("result is tick-aligned per the provided tickSize", () => {
  const result = computeSurrogateTakePrice({
    plannedEntryPrice: "100000.33",
    side: "long",
    tickSize: "0.5",
    minPrice: "0.5",
    maxPrice: "1999999.98",
  ...
  if (result.ok) {
    ...
    assert.equal(result.price, "150000.5");
  }
});
...
test("long rounds away from reference (up), short rounds away from reference (down) — not accidentally symmetric", () => {
  ...
  if (longResult.ok && shortResult.ok) {
    ...
    assert.equal(longResult.price, "150001");
    // short: 100000.01 * 0.5 = 50000.005 -> floor(tick 1) = 50000
    assert.equal(shortResult.price, "50000");
  }
});

test("a candidate beyond maxPrice (long) clamps to the nearest valid tick inside the bound rather than being rejected", () => {
  const result = computeSurrogateTakePrice({
    plannedEntryPrice: "100000",
    side: "long",
    tickSize: "0.5",
...
test("a candidate below minPrice (short) clamps to the nearest valid tick inside the bound rather than being rejected", () => {
  const result = computeSurrogateTakePrice({
    plannedEntryPrice: "100000",
    side: "short",
    tickSize: "0.5",
...
test("a synthetic instrument whose bounds leave no room for a distinct surrogate returns surrogate_unrepresentable", () => {
  const result = computeSurrogateTakePrice({
    plannedEntryPrice: "100000",
    side: "long",
    tickSize: "0.5",
  ...
  assert.deepEqual(result, { ok: false, reason: "surrogate_unrepresentable" });
});

test("clampToInstrumentBounds passes an in-bounds candidate through unchanged", () => {
  const result = clampToInstrumentBounds({
    candidate: "110000",
    reference: "100000",
    minPrice: "0.5",
    maxPrice: "1999999.98",
  ...
  assert.deepEqual(result, { ok: true, price: "110000" });
});
